...
## To use the dawid-skene EM algorithm, just do from dawid_skene import dawid_skene, and just call dawid_skene(N) [leave the default parameters as they are for now].

## This will return an I x 1 array pred of binary predictions - if pred[i] == 1, the two sentences are predicted to be in the same theme, if pred[i]==0, the two sentences are predicted to be in different themes.
